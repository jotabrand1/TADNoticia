...
git clone https://github.com/jotabrand1/TADNoticia.git
cd TADNoticia
...
| Conceito | Descrição |
|---|---|
| TAD | Separação entre interface (.h) e implementação (.c) |
| Listas Encadeadas | Duas estruturas com propósitos distintos |
| Alocação Dinâmica | Uso de `malloc` e `free` com tratamento de erros |
| Programação Defensiva | Validação de ponteiros e prevenção de memory leaks |
| Makefile | Compilação modular e automatizada |

---

## 📚 Disciplina

Desenvolvido na disciplina de **Estrutura de Dados** — Graduação em Ciência da Computação / Sistemas de Informação.
